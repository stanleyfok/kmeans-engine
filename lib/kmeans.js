...
function getEuclideanDistance(v1, v2) {
  let d = 0;
  let keys = Object.keys(Object.assign({}, v1, v2))

  keys.forEach(k => {
    //only need to consider the overlapping keys
    let delta = (v1[k] !== undefined ? v1[k] : 0) - (v2[k] !== undefined ? v2[k] : 0)
    d += delta * delta
  })
...
class Cluster {
  ...
  calculateCentroids(allVectors) {
    let newCentroid = this.vectorIds.map(id => {
      return allVectors[id]
    }).reduce((tempCentroid, v, i) => {
      let centroid = Object.assign({}, tempCentroid)

      //get unique keys
      let keys = Object.keys(Object.assign({}, centroid, v))

      keys.forEach(k => {
        let mean = (centroid[k] !== undefined) ? centroid[k] : 0
        let vk = (v[k] !== undefined) ? v[k] : 0
...
class KMeanEngine {
  ...
  init() {
    ...
    let randNums = KMeanEngine.getRandomSequence(0, this.vectors.length - 1, this.k)
  ...
  getResult() {
    return {
      ...
      clusters: this.clusters.map(cluster => {
        return _.pick(cluster, 'centroid', 'vectorIds')
      })
    }
  }
    ...
    function iterate() {
      ...
      for (let i = 0; i < self.vectors.length; i += 1) {
        ...
        for (let j = 0; j < self.clusters.length; j += 1) {
          let { centroid } = self.clusters[j]
          let v = self.vectors[i]

          let distance = getEuclideanDistance(v, centroid)
...
exports.clusterize = (vectors, k, callback) => {
  let kmeans = new KMeanEngine(vectors, k, callback)
